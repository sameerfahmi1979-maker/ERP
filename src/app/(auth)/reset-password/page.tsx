...
export default async function ResetPasswordPage() {
  ...
  try {
    ...
    if (!error && user) {
      ...
      valid = !claims.error && !session.error && session.data === true && typeof sessionId === "string" && !!await getPasswordFlow(user.id,sessionId);
    }
  } catch { /* No valid proof: render recovery guidance without changing a session. */ }
  if (!valid) return <Card className="w-full max-w-md"><CardHeader><CardTitle>Password link required</CardTitle>
    <CardDescription>This setup session is missing, expired, or already used. Open your latest invitation or request a new password reset link.</CardDescription></CardHeader>
