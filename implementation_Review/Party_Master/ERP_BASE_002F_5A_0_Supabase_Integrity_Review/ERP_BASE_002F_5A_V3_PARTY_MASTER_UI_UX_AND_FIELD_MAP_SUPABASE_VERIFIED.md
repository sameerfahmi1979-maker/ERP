...
| Finance Hold | `finance_hold` | Toggle | — | Permission-gated — finance dept hold only |
| Finance Hold Reason | `finance_hold_reason` | Textarea | — | Required if hold enabled |
...
> **SUPABASE_VERIFIED:** Column names updated to match live `audit_logs` schema. The `remarks` column does not exist. `table_name` → `entity_name`, `record_id` → `entity_id`, `user` → join to `user_profiles.full_name` via `actor_user_profile_id`.

| Column | Source |
|---|---|
| Date / Time | `audit_logs.created_at` |
| User | `user_profiles.full_name` via `audit_logs.actor_user_profile_id` |
| Action | `audit_logs.action` |
| Entity | `audit_logs.entity_name` |
| Record ID | `audit_logs.entity_id` |
| Reference | `audit_logs.entity_reference` |
| Before / After summary | `audit_logs.old_values` / `new_values` |

Filter: All `audit_logs` entries where `entity_name IN ('parties', 'party_contacts', ...)` and `entity_id = party_id` or join via child table reference.
