...
beforeEach(()=>{vi.clearAllMocks();m.ctx.mockResolvedValue({profile:{id:900001},permissionCodes:['dms.expiry.view']});m.audit.mockResolvedValue(undefined);m.provider.mockResolvedValue({config:{providerCode:'F02_OFFLINE'},sendEmail:m.send});m.send.mockResolvedValue({ok:true});});
...
it.each(['dms.expiry.view','dms.documents.view','dms.admin'])('existing %s capability remains usable',async code=>{m.ctx.mockResolvedValue({profile:{id:900001},permissionCodes:[code]});expect((await sendExpiryDocumentsEmail({...base,attachmentFormat:'none'})).success).toBe(true);});
