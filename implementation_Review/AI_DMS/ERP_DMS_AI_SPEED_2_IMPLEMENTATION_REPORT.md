...
| 1b | **Recurrence after fix 1**: the intake tab opens, then the active view snaps back to Upload Inbox (intake tab stays in the tab bar). | Two deeper Next.js behaviors, independent of `revalidatePath`: **(a)** server actions are **serialized client-side** — the `getAiIntakeStatus` poll was itself a server action, so poll calls queued behind the long-running intake action and fired in a burst around its completion; **(b)** Next.js **reverts a `router.push` performed while a server action is still in flight** — when the pending action's response arrives after the navigation, the router rebases to the URL the action was dispatched from (`/dms/inbox`). | Two-part rework: **(1)** the status poll moved from a server action to a plain **GET route handler** `src/app/api/dms/intake-status/route.ts` (parallel to the action, never touches the router; old `getAiIntakeStatus` action deleted). **(2)** the inbox client now **defers navigation until the intake action settles** (`actionSettledRef` + `pendingRedirectRef`): if the poll detects `review_pending` while the action is still pending, the redirect is parked and executed the moment the action resolves/rejects; a 10s grace timer covers the lost-response case (dev recompile, dropped connection) where the action promise never settles. |
| 1c | **Second recurrence after fix 1b** (2026-08-03): intake tab opens, then the active view returns to Upload Inbox and stays there; intermittent. | The intake action itself was now fully guarded, but the same Next.js revert rule applies to **ANY in-flight server action**, not just the intake one. The notification bell (app header, mounted on **every page**) polled `getUnreadNotificationCount()` — a server action — every 60 s, each call taking ~1.5 s. Whenever the deferred redirect to `/dms/intake/[code]` landed inside one of those windows, the poll's response rebased the router back to `/dms/inbox`. Two more interval pollers had the same latent defect: orchestration status (3 s while running) and document tags/suggestions (15 s while empty). | **All interval-polled reads moved off server actions to plain GET route handlers** (same pattern as `/api/dms/intake-status`): new `src/app/api/notifications/bell/route.ts` (unread count + dropdown list) and `src/app/api/dms/poll/route.ts` (`kind=orchestration \| documentTags \| tagSuggestions`). The route handlers invoke the existing action functions directly server-side, so auth/RLS behavior is identical. `notification-bell.tsx`, `dms-orchestration-progress-card.tsx`, and `dms-document-tags-section.tsx` now poll via `fetch`. Mutations (dismiss, mark-all-read, trigger, retry, save tags) remain server actions. **New standing rule: any TanStack Query with a `refetchInterval` must use a GET route, never a server action.** |
