...
const BASE = "/admin/master-data/finance-basics/profit-centers";
...
export function ProfitCentersTable({ profitCenters, authContext, onRefresh }: ProfitCentersTableProps) {
  ...
  const handleView = (item: ProfitCenter) => router.push(`${BASE}/record/${item.id}`);
  const handleEdit = (item: ProfitCenter) => router.push(`${BASE}/record/${item.id}?mode=edit`);
