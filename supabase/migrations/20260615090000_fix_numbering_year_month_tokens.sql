-- ============================================================================
-- Fix: Add {YYYY}, {YY}, {MM}, {DD} token support to numbering functions
-- ============================================================================
-- Bug: DMS document numbers were generated as DMS-{YYYY}-000005 instead of
--      DMS-2026-000005 because the {YYYY} token was never replaced.
-- Fix: Add date token replacement to both preview and generate functions.
-- ============================================================================
...
declare
  ...
  v_preview text;
begin
  -- Get rule by rule_code or document_type_code (alias r to avoid ambiguity)
  ...
  -- Date tokens (resolved at call time using EXTRACT to avoid quoting issues)
  v_preview := replace(v_preview, '{YYYY}', (extract(year  from now()))::int::text);
  v_preview := replace(v_preview, '{YY}',   substring((extract(year from now()))::int::text from 3));
  v_preview := replace(v_preview, '{MM}',   lpad((extract(month from now()))::int::text, 2, '0'));
  v_preview := replace(v_preview, '{DD}',   lpad((extract(day   from now()))::int::text, 2, '0'));
...
comment on function public.preview_next_reference_number is
  'Preview next reference number without consuming sequence. Supports {DOC}, {SEQ*}, {YYYY}, {YY}, {MM}, {DD} tokens.';
...
begin
  ...
  -- Date tokens (resolved at generation time — captures the year/month when the number was issued)
  v_generated_ref := replace(v_generated_ref, '{YYYY}', (extract(year  from now()))::int::text);
  v_generated_ref := replace(v_generated_ref, '{YY}',   substring((extract(year from now()))::int::text from 3));
  v_generated_ref := replace(v_generated_ref, '{MM}',   lpad((extract(month from now()))::int::text, 2, '0'));
  v_generated_ref := replace(v_generated_ref, '{DD}',   lpad((extract(day   from now()))::int::text, 2, '0'));
...
comment on function public.generate_next_reference_number is
  'Generate and consume next reference number. Concurrency-safe. Supports {DOC}, {SEQ*}, {YYYY}, {YY}, {MM}, {DD} tokens.';
