...
import { NumberingRuleFormDialog } from "./numbering-rule-form-dialog";
...
export function NumberingRulesTable({ rules }: NumberingRulesTableProps) {
  ...
  const [formMode, setFormMode] = useState<"add" | "edit" | "view" | "duplicate">("add");
  ...
  const handleView = (rule: NumberingRule) => {
  ...
  const handleDuplicate = (rule: NumberingRule) => {
    setSelectedRule(rule);
