...
it('callback uses canonical ERP origin, clears inherited fragments and never trusts request host',async()=>{
 const response=await GET(new NextRequest('https://untrusted-host.invalid/auth/confirm'));
 expect(response.headers.get('location')).toBe('https://erp.algt.net/auth/link-error?reason=invalid_invite_link#');
