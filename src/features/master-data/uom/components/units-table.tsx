...
export function UnitsTable({ units, authContext, onRefresh }: UnitsTableProps) {
  ...
  const [selected, setSelected] = useState<UnitOfMeasureWithCategory | null>(null);
  ...
  const handleView = (item: UnitOfMeasureWithCategory) => {
    setSelected(item);
    setFormMode("view");
    setIsFormOpen(true);
  };

  const handleEdit = (item: UnitOfMeasureWithCategory) => {
    setSelected(item);
    setFormMode("edit");
    setIsFormOpen(true);
  };
