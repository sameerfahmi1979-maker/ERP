-- ============================================================================
-- SQL SEED: UAE Insurance Parties Official Contact and Address Enrichment (REVIEWED)
-- Purpose: Seed and enrich UAE Insurance Policy Issuers with verified contact
...
-- ── 0. SEQUENCE COUNTER RESYNCHRONIZATION ────────────────────────────────────
-- Resync MASTER_PARTY sequence state if it is out of sync with actual parties
UPDATE public.global_numbering_sequence_states
SET 
  last_sequence_number = 83,
  next_sequence_number = 84,
  last_generated_reference = 'PTY-000083',
  updated_at = now()
WHERE numbering_rule_id = 20;
...
-- Inserts or updates the verified insurance policy issuers and seeds their official addresses/contacts.
DO $$
DECLARE
  v_party_id        BIGINT;
  v_party_code      TEXT;
...
BEGIN
  ...
  FOR v_rec IN SELECT * FROM temp_enriched_insurers LOOP
    ...
    -- Protect against multiple matching records
    SELECT count(*), max(id) INTO v_match_count, v_party_id
    FROM public.parties 
    WHERE lower(legal_name_en) = lower(v_rec.legal_name_en)
       OR remarks LIKE '%CBUAE registration no: ' || v_rec.cbuae_reg_no || '%';

    IF v_match_count > 1 THEN
      RAISE NOTICE 'HUMAN REVIEW REQUIRED: Multiple matching records found for %. Skipping from automatic script.', v_rec.legal_name_en;
      CONTINUE;
    ELSIF v_match_count = 1 THEN
      -- MODE A: Single matching party exists, enrich missing contact fields using COALESCE
  ...
  RAISE NOTICE 'Reviewed Enrichment Summary: % parties inserted, % parties updated, % addresses seeded, % contacts seeded.', 
...
SELECT 
  ...
  p.website,
  p.po_box,
  pa.street AS head_office_address,
...
-- 3. Reset the enriched fields in parties back to NULL if they were modified by this script
-- (Only clears them if they match the exact remarks patterns seeded by this script)
UPDATE public.parties
SET
  main_phone = CASE WHEN main_phone IN ('800 725262', '+971 4 282 4403', '+971 4 211 5800', '600 532626', '+971 4 605 1111', '600 522550', '600 511112', '600 5 80000', '800 SUKOON (785666)', '800 8040', '800 2244', '+971 4 3787 777', '800 98', '06 517 4444', '800 842', '06 519 5666', '+971 4 702 8500', '+971 2 694 9444', '+971 2 611 9999', '+971 9 223 3355', '600 544 040', '+971 4 601 7500', '+971 4 253 1300', '800 42924', '+971 4 319 3111', '800 DUBINS (382467)', '800 7254', '600 565 695', '800 928 2642') THEN NULL ELSE main_phone END,
  main_email = CASE WHEN main_email IN ('info@salama.ae', 'customercare@sukoontakaful.com', 'customerservice@ngiuae.com', 'customerinfo@damanhealth.ae', 'care@alliance-uae.com', 'customerrelations@takafulemarat.com', 'customerservice@insurancehouse.ae', 'info@dni.ae', 'service@sukoon.com', 'info@adnic.ae', 'customer.service@takaful.ae', 'info@unioninsurance.ae', 'info@eminsco.com', 'care@albuhaira.com', 'customercare@fidelityunited.ae', 'sico@shjins.com', 'asnic@alsagrins.ae', 'info@aldhafrainsurance.ae', 'info@alaininsurance.com', 'callcenter@fujinsco.ae', 'customercare@awnic.com', 'CustomerCare@orienttakaful.ae', 'orient@alfuttaim.ae', 'contact@hayah.com', 'info@aman.ae', 'info@dubins.ae', 'info@rakinsurance.com', 'info@methaq.ae', 'info@watania.ae') THEN NULL ELSE main_email END,
  website = CASE WHEN website IN ('https://www.salama.ae', 'https://www.sukoontakaful.com', 'https://www.ngi.ae', 'https://www.damanhealth.ae', 'https://www.alliance-uae.com', 'http://www.takafulemarat.com', 'https://www.insurancehouse.ae', 'https://www.dni.ae', 'https://www.sukoon.com', 'https://www.adnic.ae', 'https://www.takaful.ae', 'https://www.unioninsurance.ae', 'https://www.eminsco.com', 'https://albuhaira.com', 'https://fidelityunited.ae', 'https://www.shjins.com', 'http://www.alsagrins.ae', 'http://www.aldhafrainsurance.ae', 'https://www.alaininsurance.com', 'https://afnic.ae', 'https://awnic.com', 'https://www.orienttakaful.ae', 'https://www.insuranceuae.com', 'https://www.hayah.com', 'https://www.aman.ae', 'https://www.dubins.ae', 'https://www.rakinsurance.com', 'https://www.methaq.ae', 'https://www.watania.ae') THEN NULL ELSE website END,
  po_box = CASE WHEN po_box IN ('10214', '1993', '154', '128888', '5501', '57589', '129921', '1806', '5209', '839', '35335', '119227', '3856', '6000', '1888', '792', '121828', '319', '3077', '277', '45154', '183368', '27966', '63323', '157', '3027', '506', '32774', '48883') THEN NULL ELSE po_box END,
...
-- 4. Delete the parties that were newly created by this script (excluding manually matched pre-existing ones)
...
WHERE party_type_id = 54
  AND party_id IN (
    ...
    WHERE remarks LIKE 'CBUAE licensed insurance policy issuer. CBUAE register category:%'
      AND remarks NOT LIKE '%Yas Takaful%'
      -- Ensure we only delete parties that do not have other assignments or relationships
      AND id NOT IN (
        SELECT party_id FROM public.party_type_assignments WHERE party_type_id != 54
      )
  );
...
WHERE remarks LIKE 'CBUAE licensed insurance policy issuer. CBUAE register category:%'
  AND remarks NOT LIKE '%Yas Takaful%'
  AND id NOT IN (
    SELECT party_id FROM public.party_type_assignments
  );
