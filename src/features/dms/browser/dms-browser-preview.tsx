...
/**
 * DMS.BROWSER.1 — Right column: document preview + metadata strip.
 *
 * - PDFs render inline via <iframe> using /api/dms/file proxy (inline disposition)
 * - Images render via <img>
 * - Other types: download card only
 * - "Open Full Record" opens the DMS document workspace tab
 */

import { useMemo, useState } from "react";
import { format } from "date-fns";
import { Download, ExternalLink, Eye, File as FileIcon, Search } from "lucide-react";
...
import type { DmsBrowserDocument, DmsBrowserFile } from "@/server/actions/dms/browser";

interface DmsBrowserPreviewProps {
  document: DmsBrowserDocument | null;
}
...
export function DmsBrowserPreview({ document: doc }: DmsBrowserPreviewProps) {
  const { openTab } = useWorkspace();
  const [activeFileIndex, setActiveFileIndex] = useState(0);

  // Reset file tab when document changes (render-time adjustment)
  const [prevDoc, setPrevDoc] = useState<DmsBrowserDocument | null>(doc);
  if (prevDoc !== doc) {
    setPrevDoc(doc);
    setActiveFileIndex(0);
  }

  const activeFile: DmsBrowserFile | null = useMemo(() => {
    ...
    return (
      <div className="flex flex-col items-center justify-center h-full gap-3 px-6 text-muted-foreground">
        ...
        <p className="text-sm text-center max-w-xs">
          Search for a document and click a result to preview it here.
        </p>
      </div>
    );
  }
  ...
  const downloadUrl = activeFile ? `/api/dms/file?fileId=${activeFile.id}&disposition=attachment` : null;
      ...
      <div className="flex-1 min-h-0 bg-muted/20">
        {!activeFile ? (
          <div className="flex flex-col items-center justify-center h-full gap-2 text-muted-foreground px-6">
            ...
            <p className="text-xs text-center">This document has no files attached.</p>
          </div>
        ) : kind === "pdf" ? (
          <iframe
            key={activeFile.id}
