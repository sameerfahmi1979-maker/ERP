...
export function AreasTable({ areas, authContext, onRefresh }: AreasTableProps) {
  ...
  const [selectedArea, setSelectedArea] = useState<AreaZoneWithRelations | null>(null);
  ...
  const handleView = (area: AreaZoneWithRelations) => {
    setSelectedArea(area);
    setFormMode("view");
    setIsFormOpen(true);
  };

  const handleEdit = (area: AreaZoneWithRelations) => {
    setSelectedArea(area);
    setFormMode("edit");
    setIsFormOpen(true);
  };
