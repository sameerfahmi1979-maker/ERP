...
export const assetIssueReportFetcher: ReportFetcher = {
  ...
  async fetch(filters: Record<string, unknown>, _permissions: string[], db: ReportReadClient): Promise<ReportDataResult> {
    ...
    let q = db
      ...
      .select(
        `id, employee_id, asset_type, asset_name:asset_description, asset_tag:asset_reference, condition_at_issue:condition_on_issue,
         issue_date:issued_date, return_date:returned_date, return_condition:condition_on_return, asset_status:status, notes,
         employee:employees(
